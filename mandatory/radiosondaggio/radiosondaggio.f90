...
INTEGER :: iostat_input, iostat_output, n_lines, stat_p, stat_T, stat_z, stat_z_A, stat_p_A, n_layers, stat_z_B, stat_p_B
REAL(KIND=WK), ALLOCATABLE :: p(:), p_A(:), p_B(:)  ! mbar
...
ELSE
    ...
    ELSE
        ...
        ELSE
            CALL get_analyses(p, T, z, z_res, z_grid=z_A, p_grid=p_A)
        END IF
    END IF
    IF (ALLOCATED(p)) THEN
        DEALLOCATE(p)
...
ELSE
    ...
    ELSE
        ...
        ! Create pressure analyses.
        n_layers = INT((z(n_lines) - z(1)) / z_res) + 1
        ALLOCATE(z_B(n_layers), STAT=stat_z_B)
        ALLOCATE(p_B(n_layers), STAT=stat_p_B)
        IF (stat_z_B > 0) THEN
            WRITE(*, 101) 'station A grid altitude'
        ELSE IF (stat_p_B > 0) THEN
            WRITE(*, 101) 'station A grid pressure'
        ELSE
            CALL get_analyses(p, T, z, z_res, z_grid=z_B, p_grid=p_B)
        END IF
    END IF
    IF (ALLOCATED(p)) THEN
        DEALLOCATE(p)
